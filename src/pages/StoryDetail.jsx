...
export default function StoryDetail() {
      ...
      try {
        ...
        if (docSnap.exists()) {
          const storyData = {
            id: docSnap.id,
            ...docSnap.data(),
            date: docSnap.data().publishDate?.toDate().toLocaleDateString('en-US', {
              year: 'numeric',
              month: 'long',
              day: 'numeric'
            }) || docSnap.data().createdAt?.toDate().toLocaleDateString('en-US', {
              year: 'numeric',
              month: 'long',
              day: 'numeric'
            }) || 'Unknown date'
          ...
          fetchRelatedStories(storyData.category, docSnap.id);
        } else {
          console.error('No such document!');
          // Fallback to sample data if story not found
          setStory({
            id: 1,
  ...
  const fetchRelatedStories = async (category, currentStoryId) => {
    try {
      const relatedStoriesQuery = query(
        collection(db, 'stories'),
        where('category', '==', category),
        limit(3)
      );
      
      const querySnapshot = await getDocs(relatedStoriesQuery);
      const relatedStoriesData = querySnapshot.docs
        .map(doc => ({
          id: doc.id,
          ...doc.data(),
          date: doc.data().publishDate?.toDate().toLocaleDateString('en-US', {
            year: 'numeric',
            month: 'long',
            day: 'numeric'
      ...
      setRelatedStories(relatedStoriesData);
    } catch (error) {
  ...
  return (
    <MainLayout>
      <Helmet>
        <title>{story.title} - Charity NGO</title>
        <meta name="description" content={story.excerpt} />
        <meta property="og:title" content={`${story.title} - Charity NGO`} />
        <meta property="og:description" content={story.excerpt} />
        <meta property="og:type" content="article" />
        <meta property="og:image" content={story.imageUrl || story.image} />
        <meta property="article:published_time" content={story.publishDate?.toDate?.().toISOString() || story.createdAt?.toDate?.().toISOString()} />
      ...
      <div className="bg-base-200 py-8">
        <div className="container-custom">
          <div className="flex items-center mb-4">
            <Link to="/stories" className="flex items-center text-base-content/70 hover:text-primary transition-colors">
        ...
        <div className="container-custom py-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {relatedStories.map((relatedStory) => (
              <div key={relatedStory.id} className="card bg-base-100 shadow-md h-full relative group">
                ...
                <div className="card-body relative">
                  <h3 className="card-title hover:text-primary transition-colors">
                    <Link to={`/stories/${relatedStory.id}`}>{relatedStory.title}</Link>
                  </h3>
                  <p className="text-base-content/80">{relatedStory.excerpt.substring(0, 100)}...</p>
